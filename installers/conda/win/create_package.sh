...
# If in a conda env grab the path to makensis
if [[ -z $CONDA_PREFIX ]]; then
  NSIS_PATH=makensis
else
  NSIS_PATH=$CONDA_PREFIX/NSIS/makensis.exe
fi

echo "Run makensis"
cmd.exe /C "START /wait "" $NSIS_PATH /DVERSION=$VERSION /DPACKAGE_DIR=\"$COPY_DIR\" /DPACKAGE_SUFFIX=$SUFFIX /DOUTFILE_NAME=$PACKAGE_NAME /DICON_PATH=$ICON_PATH /DWORKBENCH_ICON=$WORKBENCH_ICON /DNOTEBOOK_ICON=$NOTEBOOK_ICON /DMUI_PAGE_LICENSE_PATH=$LICENSE_PATH \"$NSIS_SCRIPT\""
